...
void solve(){
    ...
    cin>>n;  
    vi a;in(a,n);   
    unordered_map<ll,ll>Left_freq;
    unordered_map<ll,ll> Right_freq;   
    for(auto i:a)Left_freq[i]++;    
    ll ans=0; 
    // fixed r==n-1 and not start l from n-1 to 0 
    //   x+1->x
    //    <- x-1 // it will become x and now it help x+1 with x in inversion
    ll curr=0;  
    looprev(i,n-1,0){
        Left_freq[a[i]]--;  
        Right_freq[a[i]]++; 
        curr+=Left_freq[a[i]+1];
        curr-=Right_freq[a[i]-1]; 
        amax(ans,curr);  
    }
    cout<<ans<<en;
