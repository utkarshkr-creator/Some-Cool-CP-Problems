...
void solve(){
    ...
    // agr a[i]<=a[i+1] -> sahi hai
    // a[i]>a[i+1]->sahi karna hai
    // ab esme se x gtate hai or wo a[i]>a[i+1] ho jta hai to humko ye nhi chahiye
    // es case me hum dekhenge ki x ko kaise bdhaye ya gtaye
    // agr orignal me a[i]<a[i+1] tha to humko x ko
    while(l<=h){
        ll mid=(l+h)/2;   
        ll flag=0;   
        loop(i,0,n-1){
