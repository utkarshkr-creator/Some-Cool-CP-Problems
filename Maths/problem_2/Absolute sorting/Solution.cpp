...
void solve(){
    ...
    // a>a[i]>a[i+1] in this case x>=ceil(a[i]+a[i+1])/2
    // b>a[i]<a[i+1] in this case x<=floor(a[i]+a[i+1])/2;
    ll max_x=INF,min_x=0;   
    loop(i,0,n-1){
         if(a[i]==a[i+1])continue;  
         else if(a[i]>a[i+1]){
            ll mid=(a[i]+a[i+1]+1)/2;
            amax(min_x,mid);
    ...
    // so x should be between max_x and min_x
    // if there is no point between them then no aswer possible
    // read tutorial for proof
